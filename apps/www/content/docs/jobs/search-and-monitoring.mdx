...
description: Find, filter, and monitor jobs in your queues to track execution, debug issues, and maintain system health.
---
...
async function analyzeJobPerformance() {
  ...
  const avgDuration = executionTimes.reduce((sum, job) => sum + job.duration, 0) / executionTimes.length;
