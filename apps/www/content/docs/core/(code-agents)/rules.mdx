...
By the end of this guide, you'll understand how rules work in modern code agents and how to use them to customize Lia's behavior for your Igniter.js project.
...
The Igniter.js includes a comprehensive set of rules organized in the `.rulesync/rules/` directory. These rules are specifically designed for multi-tenant SaaS applications built with modern web technologies.
...
To create a new custom rule for your project, you have two options: using the `/create-rule` command for guided creation, or manually creating the rule file. After creating the rule, you'll need to synchronize it with your code agent using the Igniter.js CLI.
...
```bash
npx @igniter-js/cli@latest lia generate --targets copilot,cursor,claudecode
```
