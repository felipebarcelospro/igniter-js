...
- `dev`: Start the development server with live reloading and client generation
- `build`: Compile your project for production deployment  
- `test`: Execute your test suite with filtering and watch options
- `generateSchema`: Generate type-safe client from your API router
- `generateDocs`: Create OpenAPI documentation with interactive UI
...
The Igniter.js MCP Server comes pre-configured in all official starters. For existing projects:

```bash
npm install @igniter-js/mcp-server
```

Then configure it in your main Igniter.js file:
...
### Development Tasks
- "Create tasks for implementing email verification with proper validation"
- "What tasks are currently in progress and who's working on them?"
- "Find tasks that would be good candidates for AI agent delegation"
