...
describe("Jobs Registry", () => {
  ...
  test("resolves jobs by namespace path and caches results", () => {
    const registry = createJobsRegistry(jobs, { enableCache: true });

    const first = registry.getJobByPath("user.send-email");
    const second = registry.getJobByPath("user.send-email");

    expect(first?.namespace).toBe("user");
    expect(second?.jobId).toBe("send-email");
    expect(registry.getCacheStats().hits).toBeGreaterThan(0);
  });

  test("returns null for missing namespaces or jobs", () => {
    const registry = createJobsRegistry(jobs, { enableCache: false });

    expect(registry.getJobByPath("missing.send-email")).toBeNull();
    expect(registry.getJobByPath("user.missing")).toBeNull();
  });

  test("throws for invalid job path format", () => {
    const registry = createJobsRegistry(jobs);

    expect(() => registry.getJobByPath("invalid")).toThrow(
      "Invalid job path",
    );
  });

  test("lists namespaces and jobs", () => {
    const registry = createJobsRegistry(jobs);

    expect(registry.listNamespaces()).toEqual(["user"]);
    expect(registry.listAllJobs()).toEqual(["user.send-email"]);
    expect(registry.hasJob("user.send-email")).toBe(true);
    expect(registry.getJobsByNamespace("user")).toEqual(jobs.user);
    expect(registry.getJobsByNamespace("missing")).toBeNull();
  });

  test("clears cache and resets stats", () => {
    const registry = createJobsRegistry(jobs);

    registry.getJobByPath("user.send-email");
    registry.clearCache();
  ...
  test("evicts cache entries when size limit is reached", () => {
    const registry = createJobsRegistry(jobs, { maxCacheSize: 1 });
    ...
    const otherJobs = {
      user: {
        "send-email": jobs.user["send-email"],
        "send-otp": {
          name: "Send OTP",
    ...
    const otherRegistry = createJobsRegistry(otherJobs, { maxCacheSize: 1 });
    otherRegistry.getJobByPath("user.send-email");
    otherRegistry.getJobByPath("user.send-otp");

    expect(otherRegistry.getCacheStats().size).toBe(1);
  });
});
...
describe("Jobs Proxy", () => {
  ...
  test("exposes namespace actions and management APIs", async () => {
    const registry = createJobsRegistry(jobs);
    const invoke = vi.fn().mockResolvedValue("job-123");

    const managementApi: JobsManagementApi = {
      queues: { pause: vi.fn(), resume: vi.fn() },
      job: { retry: vi.fn(), remove: vi.fn() },
      workers: { list: vi.fn() },
    } as any;

    const proxy = createJobsProxy(jobs, registry, invoke, managementApi);

    expect(proxy.$queues).toBe(managementApi.queues);
    expect(proxy.$job).toBe(managementApi.job);
    expect(proxy.$workers).toBe(managementApi.workers);

    const result = await proxy.user.sendEmail.enqueue({
      input: { email: "test@example.com" },
    } as any);

    expect(result).toBe("job-123");
    expect(invoke).toHaveBeenCalledWith(
      expect.objectContaining({ namespacedJobId: "user.sendEmail" }),
    );
  });

  test("supports fallback enqueue and bulk helpers", async () => {
    const registry = createJobsRegistry(jobs);
    const invoke = vi.fn().mockResolvedValue("job-123");
    const proxy = createJobsProxy(jobs, registry, invoke);

    const enqueueResult = await proxy.user.enqueue({
      task: "sendEmail",
      input: { email: "test@example.com" },
    });

    expect(enqueueResult).toBe("job-123");

    const bulkResult = await proxy.user.bulk([
      { jobId: "sendEmail", input: { email: "a@test.com" } },
      { jobId: "sendEmail", input: { email: "b@test.com" } },
    ]);

    expect(bulkResult).toEqual(["job-123", "job-123"]);
  });

  test("throws on missing namespace or job", () => {
    const registry = createJobsRegistry(jobs);
    const proxy = createJobsProxy(jobs, registry, vi.fn());

    expect(() => proxy.missing).toThrow("Namespace \"missing\" not found");
    expect(() => (proxy.user as any).missing).toThrow(
      "Property \"missing\" not found in namespace",
    );
  });

  test("returns undefined management APIs when not provided", () => {
    const registry = createJobsRegistry(jobs);
    const proxy = createJobsProxy(jobs, registry, vi.fn());

    expect(proxy.$queues).toBeUndefined();
    expect(proxy.$job).toBeUndefined();
    expect(proxy.$workers).toBeUndefined();
  });
});
