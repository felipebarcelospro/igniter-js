...
describe("IgniterConsoleLogger behaviors", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("supports grouping and separators", () => {
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const logger = new IgniterConsoleLogger({
  ...
  it("creates child loggers with inherited context", () => {
    ...
    const child = logger.child("Child", { feature: "test" });

    child.info("child message");

    const output = logSpy.mock.calls[0]?.[0] as string;
    expect(output).toContain("requestId=1");
    expect(output).toContain("feature=test");
  });

  it("updates logger metadata at runtime", () => {
    ...
    logger.setAppName("TestApp");
    logger.setComponent("Runtime");
    logger.setLevel(IgniterLogLevel.DEBUG);

    logger.debug("runtime");

    expect(debugSpy.mock.calls[0]?.[0]).toContain("TestApp");
  });
});
