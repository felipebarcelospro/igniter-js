...
describe("Response Processor", () => {
  ...
  describe("Processor Creation and Configuration", () => {
    ...
    it("should create processor with context and store", () => {
      const processor = IgniterResponseProcessor.init<TestContext>(
        mockStore,
        testContext,
      );
    ...
    it("should set HTTP status code", () => {
      const processor = IgniterResponseProcessor.init().status(201);

      expect(processor).toBeInstanceOf(IgniterResponseProcessor);
    });

    it("should allow chaining with status", () => {
      const processor = IgniterResponseProcessor.init().status(400).status(500);
  ...
  describe("Streaming Responses", () => {
    ...
    it("should throw error for stream without channel ID", async () => {
      await expect(
        IgniterResponseProcessor.init().stream({}).toResponse(),
  ...
  describe("Cache Revalidation", () => {
    ...
    it("should handle scope resolver errors gracefully", async () => {
      const failingScopeResolver = vi
        .fn()
        .mockRejectedValue(new Error("Scope error"));
  ...
  describe("Method Chaining and Builder Pattern", () => {
    ...
    it("should respect explicit status over error method defaults", async () => {
      // When status is explicitly set, error methods should respect that decision
      const responseWithExplicitStatus = await IgniterResponseProcessor.init()
        .status(200) // Explicit status - should be respected
        .badRequest("Validation error")
        .toResponse();

      expect(responseWithExplicitStatus.status).toBe(200); // Explicit status is preserved

      // When status is NOT explicitly set, error methods should set appropriate status
      const responseWithDefaultStatus = await IgniterResponseProcessor.init()
        .badRequest("Validation error")
        .toResponse();

      expect(responseWithDefaultStatus.status).toBe(400); // Default error status is used
    });
  ...
  describe("Data Serialization and Types", () => {
    it("should handle primitive data types", async () => {
      const responses = await Promise.all([
        IgniterResponseProcessor.init().success("string").toResponse(),
        IgniterResponseProcessor.init().success(42).toResponse(),
        IgniterResponseProcessor.init().success(true).toResponse(),
        IgniterResponseProcessor.init().success(null).toResponse(),
      ]);
  ...
  describe("Response Structure Consistency", () => {
    it("should always return consistent response structure for success", async () => {
      const responses = await Promise.all([
        IgniterResponseProcessor.init().success({ test: 1 }).toResponse(),
        IgniterResponseProcessor.init().created({ test: 2 }).toResponse(),
        IgniterResponseProcessor.init().json({ test: 3 }).toResponse(),
      ]);
    ...
    it("should always return consistent response structure for errors", async () => {
      const responses = await Promise.all([
        IgniterResponseProcessor.init().badRequest("Bad").toResponse(),
        IgniterResponseProcessor.init().unauthorized("Unauth").toResponse(),
        IgniterResponseProcessor.init().forbidden("Forbidden").toResponse(),
        IgniterResponseProcessor.init().notFound("NotFound").toResponse(),
      ]);
    ...
    it("should maintain consistent headers", async () => {
      const responses = await Promise.all([
        IgniterResponseProcessor.init().success({}).toResponse(),
        IgniterResponseProcessor.init().badRequest().toResponse(),
      ]);
