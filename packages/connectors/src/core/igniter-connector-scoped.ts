...
import type { IgniterConnectorOAuthCallbackParams } from '../types/oauth'
import type {
...
import { IgniterConnector } from './igniter-connector'
...
> {
  /** @internal */
  private manager: IgniterConnector<
  ...
  /** @internal */
  constructor(
    manager: IgniterConnector<Record<string, IgniterConnectorScopeDefinition>, TConnectors>,
    scope: string,
    identity: string
  ) {
  ...
  async list(options?: {
    ...
    const records = await this.manager.getAdapter().list(this.scopeType, this.scopeIdentity)
    ...
    for (const record of records) {
      ...
      if (options?.where?.enabled !== undefined && record.enabled !== options.where.enabled) {
        continue
      }
      ...
      const decryptedValue = await this.manager.decryptConfig(record.value as Record<string, unknown>)
  ...
  async get<K extends keyof TConnectors & string>(
    ...
    const record = await this.manager.getAdapter().get(this.scopeType, this.scopeIdentity, connectorKey)
  ...
  async connect<K extends keyof TConnectors & string>(
    ...
    const validationResult = await IgniterConnectorSchema.validate(connector.configSchema, config)
    if (!validationResult.success) {
      const errorMessage = validationResult.errors?.map((e) => e.message).join(', ') || 'Invalid config'
    ...
    if (connector.hooks.onValidate) {
      await connector.hooks.onValidate({ config: validConfig, scope: this.scopeType, identity: this.scopeIdentity })
    }
    ...
    if (connector.oauth) {
      ...
      return response as unknown as ExtractConnectorConnectOutput<TConnectors[K]>
    }
    ...
    await this.manager.getAdapter().save(this.scopeType, this.scopeIdentity, connectorKey, encryptedConfig, true)
    
    // Side Effect: Call onConnect hook
    const hooks = this.manager.getHooks()
    if (hooks.onConnect) {
      await hooks.onConnect({ connector: connectorKey, scope: this.scopeType, identity: this.scopeIdentity, config: validConfig })
    }
    
    // Event Handling: Emit connected event
    await this.emitEvent({ type: 'connected', connector: connectorKey, scope: this.scopeType, identity: this.scopeIdentity, timestamp: new Date() })
  }
   ...
   * ```
   */
  async disconnect<K extends keyof TConnectors & string>(connectorKey: K): Promise<void> {
    // Database Query: Delete record
    await this.manager.getAdapter().delete(this.scopeType, this.scopeIdentity, connectorKey)
    
    // Side Effect: Call onDisconnect hook
    const hooks = this.manager.getHooks()
    if (hooks.onDisconnect) {
      await hooks.onDisconnect({ connector: connectorKey, scope: this.scopeType, identity: this.scopeIdentity })
    }
    
    // Event Handling: Emit disconnected event
    await this.emitEvent({ type: 'disconnected', connector: connectorKey, scope: this.scopeType, identity: this.scopeIdentity, timestamp: new Date() })
  }
   ...
   * ```
   */
  async toggle<K extends keyof TConnectors & string>(connectorKey: K, enabled?: boolean): Promise<void> {
    // Database Query: Get current state if not explicitly provided
    if (enabled === undefined) {
      const record = await this.manager.getAdapter().get(this.scopeType, this.scopeIdentity, connectorKey)
      if (!record) throw IgniterConnectorError.connectorNotConnected(connectorKey)
      enabled = !record.enabled
    }
    
    // Database Query: Update enabled state
    await this.manager.getAdapter().update(this.scopeType, this.scopeIdentity, connectorKey, { enabled })
  ...
  > {
    const self = this
    return {
      async call(input: InferActionInput<TConnectors, K, A>): Promise<IgniterConnectorActionResult<InferActionOutput<TConnectors, K, A>>> {
        try {
          const result = await self.executeActionInternal(connectorKey, actionKey, input)
  ...
  on(handler: IgniterConnectorEventHandler): IgniterConnectorEventSubscription {
    ...
    return { unsubscribe: () => { this.localEventHandlers.delete(handler) } }
  }
  ...
  private async executeActionInternal<TInput, TOutput>(
    ...
    if (!action) throw IgniterConnectorError.actionNotFound(connectorKey, actionKey)
    
    // Database Query: Get connector instance
    const instance = await this.get(connectorKey as keyof TConnectors & string)
    if (!instance) throw IgniterConnectorError.connectorNotConnected(connectorKey)
    if (!instance.enabled) throw IgniterConnectorError.connectorDisabled(connectorKey)
    ...
    if (connector.hooks.onContext && instance.config) {
      context = await connector.hooks.onContext({ config: instance.config, scope: this.scopeType, identity: this.scopeIdentity })
    }
    ...
    try {
      ...
      const result = await action.handler({
        ...
        oauth: instance.oauth ? {
          ...
          userInfo: instance.oauth.userInfo ? {
            ...
            avatar: instance.oauth.userInfo.avatar as string | undefined,
          } : undefined,
      ...
      const hooks = this.manager.getHooks()
      if (hooks.onError) {
        await hooks.onError({ error: error as Error, connector: connectorKey, scope: this.scopeType, identity: this.scopeIdentity, action: actionKey })
      }
  ...
  private async ensureValidTokens(
    ...
    if (!IgniterConnectorOAuthUtils.canRefresh(tokens)) throw IgniterConnectorError.oauthTokenExpired(connectorKey)
    
    const oauthHandler = this.manager.getOAuthHandler(connectorKey)
    if (!oauthHandler) throw IgniterConnectorError.oauthNotConfigured(connectorKey)
    if (!tokens.refreshToken) throw IgniterConnectorError.oauthTokenExpired(connectorKey)
    ...
    const updatedConfig = { ...(instance.config as Record<string, unknown>), oauth: updatedOauth }
    const encryptedConfig = await this.manager.encryptConfig(updatedConfig)
    
    await this.manager.getAdapter().update(this.scopeType, this.scopeIdentity, connectorKey, { value: encryptedConfig })
  ...
  /** @internal */
  private async emitEvent(event: IgniterConnectorEvent): Promise<void> {
