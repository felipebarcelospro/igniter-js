...
describe('OtlpTransportAdapter', () => {
  ...
  it('should send OTLP logs via fetch', async () => {
    const adapter = OtlpTransportAdapter.create({ url: 'http://otel-collector:4318/v1/logs' })
    const envelope = createEnvelope()
    
    await adapter.handle(envelope)
    
    expect(fetch).toHaveBeenCalledWith('http://otel-collector:4318/v1/logs', expect.objectContaining({
      method: 'POST',
  ...
  it('should map severity correctly', async () => {
    ...
    for (const [level, severity] of levels) {
      ...
      const body = JSON.parse((fetch as any).mock.calls[levels.indexOf([level, severity])][1].body)
      expect(body.resourceLogs[0].scopeLogs[0].logRecords[0].severityNumber).toBe(severity)
    }
  })
  ...
  it('should include attributes in OTLP format', async () => {
    ...
    expect(attributes).toContainEqual({ key: 'str', value: { stringValue: 'value' } })
    expect(attributes).toContainEqual({ key: 'num', value: { doubleValue: 123 } })
    expect(attributes).toContainEqual({ key: 'bool', value: { boolValue: true } })
  })
})
