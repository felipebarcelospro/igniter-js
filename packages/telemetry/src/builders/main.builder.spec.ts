...
describe("IgniterTelemetryBuilder", () => {
  ...
  describe("addEvents()", () => {
    it("should add events descriptor", () => {
      const events = IgniterTelemetryEvents.namespace("app")
    ...
    it("should throw on duplicate namespace", () => {
      ...
      expect(() =>
        IgniterTelemetryBuilder.create().addEvents(events1).addEvents(events2),
      ).toThrow(IgniterTelemetryError);
    });
  });
  ...
  describe("addTransport()", () => {
    ...
    it("should allow duplicate transport key", () => {
      ...
      // We can't easily inspect internal state here without casting to any,
      // but the fact it didn't throw is what we want to verify for now.
    });

    it("should support adding transport without type key", () => {
      const transport = createMockTransport("logger");
      const builder = IgniterTelemetryBuilder.create().addTransport(transport);
