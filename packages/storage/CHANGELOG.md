...
- Standardized telemetry registry and event attributes with `@igniter-js/telemetry`.
- Mock adapter tests and utility coverage (including try-catch).

### Changed

- Aligned public entrypoint to `IgniterStorage.create()` via builder alias.
- Reworked barrel exports and browser shim mappings for server-only safety.
- Updated documentation to reflect current telemetry patterns and exports.
