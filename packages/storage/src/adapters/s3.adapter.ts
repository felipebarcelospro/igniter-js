...
import { IgniterStorageTryCatch } from "../utils/try-catch";
import {
...
export class IgniterS3StorageAdapter extends IgniterStorageAdapter {
  ...
  async put(
    ...
    if (body instanceof Readable) {
      ...
      const result = await IgniterStorageTryCatch.run(upload.done());
      if (result.error) {
        await IgniterStorageTryCatch.run(
          this.client.send(
  ...
  async exists(key: string): Promise<boolean> {
    ...
    const response = await IgniterStorageTryCatch.run(
